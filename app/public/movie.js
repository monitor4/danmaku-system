    ...
    if (progress.mousedown === 0) {
      ...
      while (
        ...
        for (i = 0; i < danmaku.path.length; i++) {
          if (curTime > danmaku.path[i].delay && $("#audio-box").width() / total_width * speed / 1000 + curTime > danmaku.path[i].time) {
            road.index = i;
            break;
          }
          else if (road.time > - $("#audio-box").width() / total_width * speed / 1000 - curTime + danmaku.path[i].time) {
            road.index = i;
            road.time = - $("#audio-box").width() / total_width * speed / 1000 - curTime + danmaku.path[i].time
          }
        }
        $span.css('top', 10 + 40 * road.index);
        danmaku.path[road.index].time = curTime + speed / 1000;
        danmaku.path[road.index].delay = curTime + speed * $span.width() / total_width / 1000
        $span.animate(
          {
            left: -1 * $span.width()
          },
